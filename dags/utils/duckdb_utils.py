...
def get_minio_config() -> dict[str, Any]:
    ...
    return {"endpoint": endpoint, "access_key": access_key, "secret_key": secret_key, "use_ssl": use_ssl}


class DuckDBValidator:
    def __init__(self, minio_config: dict[str, Any] | None = None):
    ...
    def read_parquet_from_minio(self, bucket: str, key_pattern: str, columns: list[str] | None = None) -> pd.DataFrame:
        """
    ...
    def validate_schema(
        self, df: pd.DataFrame, expected_columns: list[str], required_columns: list[str] | None = None
    ) -> dict[str, Any]:
        """
        ...
        if required_columns:
            ...
            if not columns_with_nulls.empty:
                result["valid"] = False
                result["errors"].append(f"Null values in required columns: {dict(columns_with_nulls)}")

        return result

    def validate_data_types(self, df: pd.DataFrame, type_spec: dict[str, str]) -> dict[str, Any]:
        """
        ...
        for col, expected_type in type_spec.items():
            ...
            if expected_normalized not in actual_dtype:
                result["valid"] = False
                result["errors"].append(f"Column '{col}': expected {expected_type}, got {actual_dtype}")
    ...
    def validate_uniqueness(self, df: pd.DataFrame, columns: list[str]) -> dict[str, Any]:
        ...
        if not duplicates.empty:
            result["valid"] = False
            result["errors"].append(f"Found {len(duplicates)} duplicate rows in columns: {columns}")

        return result

    def run_full_validation(self, df: pd.DataFrame, schema_spec: dict[str, Any]) -> dict[str, Any]:
        """
        ...
        if "expected_columns" in schema_spec:
            col_result = self.validate_schema(df, schema_spec["expected_columns"], schema_spec.get("required_columns"))
        ...
        if "value_ranges" in schema_spec:
            for column, range_spec in schema_spec["value_ranges"].items():
                range_result = self.validate_value_range(df, column, range_spec.get("min"), range_spec.get("max"))
...
def create_validator(minio_config: dict[str, Any] | None = None) -> DuckDBValidator:
    """Factory function to create a DuckDB validator."""
    return DuckDBValidator(minio_config)


def validate_parquet_from_minio(
    bucket: str, key_pattern: str, schema_spec: dict[str, Any], minio_config: dict[str, Any] | None = None
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if df.empty:
            return {"valid": False, "errors": ["No data found in MinIO path"], "dataframe": df}
