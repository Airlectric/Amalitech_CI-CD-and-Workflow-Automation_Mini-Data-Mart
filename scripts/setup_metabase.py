...
def setup_metabase():
    ...
    if r.status_code != 200:
        ...
        r = session.post(f"{METABASE_URL}/api/session", json={"email": EMAIL, "password": PASSWORD})
        if r.status_code != 200:
            # Try default admin
            r = session.post(f"{METABASE_URL}/api/session", json={"email": "admin@metabase.com", "password": "admin"})
...
def create_collection(session, name):
    r = session.post(f"{METABASE_URL}/api/collection", json={"name": name, "color": "#509EE3"})
    if r.status_code in [200, 201]:
...
def create_question(session, db_id, name, query, collection_id=None):
    payload = {
        ...
        "dataset_query": {
            "type": "query",
            "query": {"source_table": None, "aggregation": ["count"], "breakout": []},
            "database": db_id,
        },
        "display": "table",
    ...
    if sql.startswith("select"):
        ...
        payload["dataset_query"]["native"] = {"query": query, "template_tags": {}}
...
def main():
    ...
    questions = [
        (
            "Daily Sales",
            "SELECT sale_date, total_transactions, net_revenue FROM gold.daily_sales ORDER BY sale_date DESC LIMIT 30",
        ),
        (
            "Product Performance",
            "SELECT product_name, category, total_revenue, number_of_transactions FROM gold.product_performance ORDER BY total_revenue DESC LIMIT 20",
        ),
        (
            "Customer Analytics",
            "SELECT customer_name, customer_tier, total_revenue, total_purchases FROM gold.customer_analytics ORDER BY total_revenue DESC LIMIT 20",
        ),
        (
            "Store Performance",
            "SELECT store_location, region, total_revenue, total_transactions FROM gold.store_performance ORDER BY total_revenue DESC",
        ),
        (
            "Category Insights",
            "SELECT category, total_revenue, total_products_sold, average_order_value FROM gold.category_insights ORDER BY total_revenue DESC",
        ),
    ]
    ...
    print("Database: airflow (PostgreSQL)")
