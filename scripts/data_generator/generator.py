...
def generate_schema_invalid_data(num_rows: int = 1000) -> pd.DataFrame:
    ...
    timestamps = [
        start_date + timedelta(days=random.randint(0, 365)) for _ in range(num_rows)
    ]
    
    schema_modes = [
        "missing_required_columns",
        "wrong_column_types", 
        "extra_unknown_columns",
        "missing_entire_column"
    ]
