...
def run_tests():
    ...
    try:
        ...
        print(f"  - generate_sales_data(clean): {len(df)} records OK")

        # Test dirty data
        df = generate_sales_data(num_rows=100, mode="dirty")
        print(f"  - generate_sales_data(dirty): {len(df)} records OK")

        # Test duplicates
        df = generate_sales_data(num_rows=100, mode="duplicates")
        print(f"  - generate_sales_data(duplicates): {len(df)} records OK")
    ...
    try:
        ...
        hook.execute_query("SELECT 1 as test")
        print("  - PostgresLayerHook: connection OK")
